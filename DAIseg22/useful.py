...
def main_read2(file2): # return the dictionary with REF,ALT, Outgroup and Archaic allels. And Ancestral allles (if possible)
    ...
    for i in range(1,len(lns)):
        ...
        arch1= m[5].replace('.','')
        if len(arch1)!=0:
            arch1=[int(k) for k in arch1.split(',')]
        else:
            arch1=[]

        arch2= m[6].replace('.','')
        if len(arch2)!=0:
            arch2=[int(k) for k in arch2.split(',')]
        else:
            arch2=[]        
        ...
        if m[3]=='.': 
            s=''
            nested_dict[int(m[0])]={'REF': m[1], 'ALT':m[2], 'Outgroup': out, 'Archaic1': arch1, 'Archaic2':arch2, 'Obs': o  }
            
        else:
            s=m[3]   
            nested_dict[int(m[0])]={'REF': m[1], 'ALT':m[2], 'Outgroup': out, 'Archaic1': arch1, 'Archaic2': arch2,  'Obs': o, 'AA': s  }
