...
def initB(mu, L, t_OOA, t_ANC, t1, t2, n_st) -> np.array:
    ...
    P_nd_split[0] = np.exp(-lambda_nd_split)
    ...
    for i in range(1, n_st):
        ...
        P_nd_split[i] = P_nd_split[i - 1] * lambda_nd_split / i
    ...
    for i in range(n_st):
        for j in range(n_st):
            for k in range(n_st):
            # EU: P_OOA(i) * P_ANC(j)
                B[0][i][j][k] = P_OOA[i] * P_ANC[j] * P_ANC[k]
                
                # ND1: P_ANC(i) * P_t1(j)
                B[1][i][j][k] = P_ANC[i] * P_t1[j] * P_nd_split[k]
                
                # ND2: P_ANC(i) * P_t2(j)
                B[2][i][j][k] = P_ANC[i] * P_nd_split[j]* P_t2[k]
...
def viterbi_modified(V, initial_distribution, a,  b):
    ...
    omega[0, :] = np.log(initial_distribution * b[:, V[0][0],V[0][1], V[0][2]])
    ...
    for t in range(1, T):
        for j in range(M):
            ...
            probability = omega[t - 1] + np.log(a[:, j]) + np.log(b[j, V[t][0], V[t][1], V[t][2]])
